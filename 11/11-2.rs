...
fn exec_op(old: i128, command: &str) -> i128 {
    let expr = command.replace("old", &old.to_string());
    let mut ans = 0;
    // assume only two numbers in the expr
    if expr.contains("+") {
        ans = expr.split(" + ").map(|i| i.parse::<i128>().unwrap()).sum();
    }
    else if expr.contains("*") {
        ans = expr.split(" * ").map(|i| i.parse::<i128>().unwrap()).fold(1, |acc, x| acc * x);
    }
    ans
}
...
fn main() {
    ...
    monkeys.sort_by_key(|m| -(m.n_inspection as i128));
    println!("{:?}", monkeys);
